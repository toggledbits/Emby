...
* In `urn:micasaverde-com:serviceId:MediaNavigation1`
  ...
  * The `Stop` action will stop (completely) whatever media is playing on the client. There is no full-queue resume function, currently (see "Caveats" above), but you can resume the one media item last known to be playing using the `ResumeMedia` action, below.
