...
function actionSessionResumeMedia( pdev, restart, bookmark )
	assert(luup.devices[pdev].device_type==SESSIONTYPE)
	local sess = luup.devices[pdev].id
	local server = getVarNumeric( "Server", 0, pdev, SESSIONSID )
	...
	if bookmark then
		...
		state = (bookmarks[ bookmark ] or {}).position or ""
		if state == "" then
...
--[[ Bookmark the current media playing and last checkpoint position. This
     allows the user to later pass the bookmark to ResumeMedia on a session,
	 which will continue the bookmarked media from the saved position.
	 Bookmarks are stored on the server, and are therefore server-global, so
	 it is possible to resume a bookmark on any session belonging to that 
	 server (i.e. you can continue a movie from where you left off on any
	 device, not the just one on which it was previously paying).
--]]
function actionSessionBookmarkMedia( pdev, name )
	assert(luup.devices[pdev].device_type==SESSIONTYPE)
	if name == nil or tostring(name) == "" then
		L({level=1,msg="Invalid bookmark name: %1"}, name)
		return false
	end
	local server = getVarNumeric( "Server", 0, pdev, SESSIONSID )
